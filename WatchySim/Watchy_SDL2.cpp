/*
 * Watchy_SDL2.cpp - Linux/SDL2 implementation of Watchy
 */
...
Watchy::Watchy() : WIFI_CONFIGURED(false), BLE_CONFIGURED(false), currentVoltage(4.2f) {
    resetTime();
    
    // Initialize weather data
    currentWeather.temperature = WATCHY_DEFAULT_TEMP;
    currentWeather.weatherConditionCode = 800; // Clear
...
// DisplaySim implementation
void DisplaySim::setContext(GraphicsContext* ctx) {
    this->context = ctx;
}
    ...
    for (int currentY = 0; currentY < h; currentY++) {
        for (int currentX = 0; currentX < w; currentX++, bitOffset = bitOffset >> 1) {
            ...
            if (bitmap[bitmapOffset] & bitOffset) {
                context->setPixel(x + currentX,
                                y + currentY, 
                                colorFromGxEPD(color));
            }
        }
    }
}
...
void DisplaySim::fillScreen(uint16_t color) {
    if (!context) {
        std::cerr << "[ERROR] DisplaySim::fillScreen called with null context!" << std::endl;
        return;
    }
    uint32_t fillColor = colorFromGxEPD(color);
    // In SDL2 version, we don't use offsets since we're drawing directly to the display texture
    context->fillRect(0, 0, DISPLAY_WIDTH, DISPLAY_HEIGHT, fillColor);
}
...
void DisplaySim::println(uint8_t number) {
    ...
    snprintf(buffer, _MAX_INT_DIG, "%u", number);
    println(buffer);
}
...
void DisplaySim::println(int32_t number) {
    char buffer[_MAX_INT_DIG];
    snprintf(buffer, _MAX_INT_DIG, "%d", number);
    println(buffer);
}
...
void DisplaySim::drawPixel(int16_t x, int16_t y, uint16_t color) {
    context->setPixel(x, y, colorFromGxEPD(color));
}
